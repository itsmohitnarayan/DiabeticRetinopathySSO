...
def build_slstm_model(input_shape, params):
    input_size = input_shape[1]  # Get the input size from the input shape
    hidden_size = int(params[0])  # Get the hidden size from the parameters
    num_layers = 3  # Number of LSTM layers
    dropout = params[1]  # Get the dropout rate from the parameters
    
    model = SLSTM(input_size, hidden_size, num_layers, dropout)  # Create an instance of the SLSTM model
...
def fitness_function(params, X_train, y_train, X_val, y_val):
    units, dropout = params  # Unpack the parameters
    if units <= 0 or dropout < 0 or dropout > 1:  # Check for invalid parameters
        return float('inf')  # Return a high error rate for invalid parameters

    hidden_size = int(units)  # Convert units to integer
    if hidden_size <= 0:  # Check for invalid hidden size
        return float('inf')  # Return a high error rate for invalid hidden size

    model, criterion, optimizer = build_slstm_model((X_train.shape[1], X_train.shape[2]), params)  # Build the SLSTM model
    model = model.to(X_train.device)  # Move the model to the appropriate device
    
    # Training loop for one epoch
    model.train()  # Set the model to training mode
    optimizer.zero_grad()  # Zero the gradients
    outputs = model(X_train)  # Get model outputs
    ...
    with torch.no_grad():  # Disable gradient calculation
        val_outputs = model(X_val)  # Get validation outputs
        val_outputs = (val_outputs > 0.5).float()  # Convert probabilities to binary predictions
        error_rate = 1 - accuracy_score(y_val.cpu(), val_outputs.cpu())  # Calculate error rate
...
def modified_sparrow_search(X_train, y_train, X_val, y_val, num_agents=10, max_iter=20):
    ...
    population = np.random.uniform(low=[50, 0.1], high=[200, 0.5], size=(num_agents, dim))
...
# Function to read annotations from Excel files
...
if __name__ == "__main__":
    ...
    # Directory containing your images and annotations
    image_directory = r'D:\mohit\DiabeticRetinopathySSO\messidor'
    annotation_files = [os.path.join(image_directory, f'Annotation_Base{i}.xls') for i in range(11, 35)]
    ...
    # Read annotations
    annotations = read_annotations(annotation_files)

    # Get list of all image file paths and their corresponding labels
    image_paths = []
    labels = []
    for i in range(11, 35):
        base_dir = os.path.join(image_directory, f'Base{i}')
        if os.path.exists(base_dir):
            for filename in os.listdir(base_dir):
                if filename.endswith('.tif'):
                    image_paths.append(os.path.join(base_dir, filename))
                    labels.append(annotations.loc[annotations['Image name'] == filename, 'Retinopathy grade'].values[0])
        else:
    ...
    for epoch in tqdm(range(1000), desc="Training model"):  # Increased epochs to 50
